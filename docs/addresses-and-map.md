...
At zoom 12 and below, display source-record counts for CBD, GBD, YBD, Seoul Others and BBD; the overview positions represent regions, not building coordinates. Location-pending assets remain in regional counts and are disclosed separately. Higher zooms group confirmed coordinates by screen-scale proximity, separating as the user zooms in. Clicking a group zooms in; dense/collocated records can be individually selected from a list at zoom 19+. Details return to the previous camera. Counts represent source asset records, which can include multiple towers, interests and development records at one address.
